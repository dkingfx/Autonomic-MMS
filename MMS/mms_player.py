...
https://home-assistant.io/components/media_player.mms/

mms rest API: https://mms.github.io/docs/API/REST_API.html
"""
import asyncio
...
async def async_setup_platform(hass, config, async_add_entities,
    ...
    entity = mms(name, host, port, hass)

    hass.data[DATA_VOLUMIO][ip_addr] = entity
    async_add_entities([entity])


class mms(MediaPlayerDevice):
    """mms Player Object."""
    ...
    async def send_mms_msg(self, method, params=None):
        """Send message."""
        ...
        try:
            ...
            _LOGGER.error(
                "Failed communicating with mms '%s': %s",
    ...
    async def async_update(self):
        """Update state."""
        resp = await self.send_mms_msg('getState')
    ...
    def async_media_next_track(self):
        ...
        return self.send_mms_msg('commands', params={'cmd': 'next'})

    def async_media_previous_track(self):
        """Send media_previous command to media player."""
        return self.send_mms_msg('commands', params={'cmd': 'prev'})

    def async_media_play(self):
        """Send media_play command to media player."""
        return self.send_mms_msg('commands', params={'cmd': 'play'})

    def async_media_pause(self):
        """Send media_pause command to media player."""
        if self._state['trackType'] == 'webradio':
            return self.send_mms_msg('commands', params={'cmd': 'stop'})
        return self.send_mms_msg('commands', params={'cmd': 'pause'})

    def async_set_volume_level(self, volume):
        """Send volume_up command to media player."""
        return self.send_mms_msg(
            'commands', params={'cmd': 'volume', 'volume': int(volume * 100)})

    def async_volume_up(self):
        """Service to send the mms the command for volume up."""
        return self.send_mms_msg(
            'commands', params={'cmd': 'volume', 'volume': 'plus'})

    def async_volume_down(self):
        """Service to send the mms the command for volume down."""
        return self.send_mms_msg(
    ...
    def async_clear_playlist(self):
        ...
        self._currentplaylist = None
        return self.send_mms_msg('commands',
                                     params={'cmd': 'clearQueue'})

    @Throttle(PLAYLIST_UPDATE_INTERVAL)
    async def _async_update_playlists(self, **kwargs):
        """Update available mms playlists."""
        self._playlists = await self.send_mms_msg('listplaylists')
